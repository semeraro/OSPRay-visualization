...
int main(int argc, const char** argv) {
    ...
    std::string materialType{"obj"};
    std::string firstframe{"firstobjframe.ppm"};
    std::string accumframe{"accumulatedobjframe.ppm"};
    // sphere data
    ...
    // when all the stuff is in the world that too is committed. 
    {
        ...
        light.setParam("angularDiameter",10.0f); // sun
        ...
        rkcommon::utility::writePPM(firstframe, imgSize.x, imgSize.y, fb);
        framebuffer.unmap(fb);
        std::cout << "rendering initial frame to " + firstframe << std::endl;
        ...
        rkcommon::utility::writePPM(
            accumframe, imgSize.x, imgSize.y, fb);
        framebuffer.unmap(fb);
        std::cout << "rendering 10 accumulated frames to " + accumframe
                  << std::endl;
    }
    // turn it off
    ospShutdown();
