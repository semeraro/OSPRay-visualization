...
int main(int argc, const char** argv) {
    ...
    // when all the stuff is in the world that too is committed. 
    {
        ...
        rkcommon::utility::writePPM("alloyfirstframe.ppm", imgSize.x, imgSize.y, fb);
        framebuffer.unmap(fb);
        std::cout << "rendering initial frame to alloyfirstframe.ppm" << std::endl;
        ...
        rkcommon::utility::writePPM(
            "alloyaccumulatedframe.ppm", imgSize.x, imgSize.y, fb);
        framebuffer.unmap(fb);
        std::cout << "rendering 10 accumulated frames to alloyaccumulatedframe.ppm"
                  << std::endl;
    }
    // turn it off
    ospShutdown();
