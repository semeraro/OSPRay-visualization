...
int main(int argc, const char** argv) {
    ...
    // when all the stuff is in the world that too is committed. 
    {
        ...
        else if(materialType.compare("alloy")==0) {
            vec3f color{0.0f,0.f,1.f};
            mat.setParam("color",color);
        }
        else if(materialType.compare("glass")==0) {
            float eta{1.5f};
            vec3f attenuationColor{0.8f,0.0f,0.2f};
            float attenuationDistance{10.0f};
            mat.setParam("eta",eta);
            mat.setParam("attenuationColor",attenuationColor);
            mat.setParam("attenuationDistance",attenuationDistance);
        }
        else {
            std::cout << "strange material" << std::endl;
        }
        ...
        light.setParam("angularDiameter",0.53); // sun
        light.setParam("intensity",1.0f);
        light.setParam("intensityQuantity",OSP_INTENSITY_QUANTITY_IRRADIANCE);
        light.commit();
        ...
        rkcommon::utility::writePPM("glassfirstframe.ppm", imgSize.x, imgSize.y, fb);
        framebuffer.unmap(fb);
        std::cout << "rendering initial frame to glassfirstframe.ppm" << std::endl;
        ...
        rkcommon::utility::writePPM(
            "glassaccumulatedframe.ppm", imgSize.x, imgSize.y, fb);
        framebuffer.unmap(fb);
        std::cout << "rendering 10 accumulated frames to glassaccumulatedframe.ppm"
                  << std::endl;
    }
    // turn it off
    ospShutdown();
